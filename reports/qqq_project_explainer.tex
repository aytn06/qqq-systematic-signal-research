...
artifact generation, rather than as a one-off trading notebook.
...
they are not identical. The public repository ships with a sanitized sample
...
Because the public sample begins in 2018, the committed artifact pack adapts the
...
supposed to reproduce the original headline Sharpe. The public sample exists so
...
is why it becomes the strongest trend representative on the public sample.
...
modeled as one-way cost per unit turnover, with the public artifact pack using a
...
On the committed public sample, that process leaves three sleeves, which can be
...
than the passive benchmark. In other words, the public sample suggests that
...
that a public sample is not strong enough to support a pretty headline. In fact,
...
So the honest interpretation of the project is the following. The public repo
...
diagnostics, and artifact generation. The original private-run summary is what
...
The strongest inference is not that the public sample proves a durable edge. The
...
minor implementation details. An earlier version of the artifact generation path
...
The public repository is careful about its own limits. The public data is
...
interpretation. This is a research framework and documented research artifact,
...
transaction-cost accounting, public artifacts, and honest retention of
unflattering public-sample results. That is precisely why it works well as a
public-facing project. It does not ask the reviewer to take the process on
