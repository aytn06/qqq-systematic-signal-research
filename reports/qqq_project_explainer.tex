...
\section*{The Data and the Repo-versus-Original Distinction}

One of the most important things to understand about the repository is that it
contains both a runnable research framework and a documented summary of the
original Quanta QR Fellowship finalist submission. These are related, but they
are not identical. The repository ships with an included dataset spanning
\texttt{2018-01-02} through \texttt{2025-06-30}. At minimum,
...
plots without redistributing challenge-provided data.
...
Because the included sample begins in 2018, the committed results adapt the
split in a transparent way:

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Repo split} & \textbf{Dates} & \textbf{Purpose} \\
\midrule
...
Holdout & 2023-01-01 to 2025-06-30 & Final repo out-of-sample evaluation \\
\bottomrule
\end{tabular}
\end{center}

This distinction matters because it explains why the repo results are not
...
and that is part of what makes it credible. The included-dataset final ensemble
did not produce positive holdout Sharpe. The medium-term trend sleeve is the
cleanest individual performer in the repo dataset, but even that sleeve has only slightly
...
that an included dataset is not strong enough to support a pretty headline. In fact,
...
in the original challenge summary report because the longer challenge-provided
dataset is not redistributed. The preserved challenge materials report that the final
...
This GitHub repo was assembled later from my private research archive, which is
why some supporting summaries and evidence notes have newer git dates than the
...
turned out to be near-duplicates of \texttt{rsi\_deep\_value} on the included
...
repo ensemble is now selected with a validation-only score. This is one of the
...
The repository is careful about its own limits. The included dataset is
shorter than the intended full-history design. DXY and
...
summary rather than a fully reproducible raw historical backtest. The repo
itself was assembled later from my private research archive, which is why some
supporting notes have newer git dates than the original project period. None of these
...
unflattering included-dataset results. That is precisely why it works well as a
 project. It does not ask the reader to take the process on
faith. It shows the process directly, and then clearly distinguishes the repo
implementation from the stronger private-run result that originally motivated it.
